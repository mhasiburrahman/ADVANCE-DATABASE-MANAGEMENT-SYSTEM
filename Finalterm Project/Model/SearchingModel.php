...
function problem8(){
    $conn = connection();
    $sql = oci_parse($conn,"select p.* ,c.* from product p,catagory c where p.catagory_id=c.catagory_id and cost_price<(select cost_price from product where p_name='Marble Sculpture')
        ") ;
    $res = oci_execute($sql);
    return $sql;
}
